...
const AppItem = ({ name, imgSrc }: AppItemType) => (
  <Link href="/" className="flex flex-1 flex-col items-center gap-2 p-4 rounded-2xl hover:bg-slate-700">
    <Avatar size="medium">
...
const FlowItem = ({ name, imgSrc }: FlowItemType) => (
  <Link href="/" className="flex flex-1 flex-col items-center aspect-square gap-2 p-4 rounded-2xl hover:bg-slate-700">
...
export const NavigatorMenuInitialContent = ({ data }: NavigatorMenuInitialContentProps) => (
  <div className="size-full space-y-2 p-4">
    ...
    <div className="space-y-2">
      <h2 className="text-lg text-aqua-500 font-medium">Elements</h2>
      <div className="flex flex-wrap gap-2">
