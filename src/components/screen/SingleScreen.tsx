        ...
        >
            ...
            >
                ...
                <motion.div
                    ...
                    className={cn("absolute top-4 left-4 group/copy h-9 w-9 border-4 border-slate-200 z-40 rounded-xl flex items-center justify-center cursor-pointer invisible group-hover/item:visible", checked && 'visible border-orange-500 text-orange-500')}
                >
                    {checked &&
                        <svg
                            className='w-4 h-3'
...
const CollectionDropdown = () => {
    ...
    return (
        <div className="absolute top-16 bg-slate-900 p-3 w-[88%] z-50  rounded-xl invisible group-hover/item:visible h-auto transition-all duration-500">
            ...
            {!showCreate &&
                <>
                    <input type='text' className='bg-slate-800 font-normal w-full px-4 h-10 rounded-full' placeholder='Search Collections' />
...
const CreateCollection = () => {
    return (
        <div className='mx-2 mb-2'>
            ...
            <input type='text' className='bg-slate-800 font-normal tracking-wider w-full px-4 h-10 mt-1 rounded-lg' placeholder='Friends...' />
        </div>
    )
}
...
const DropdownCell = ({ children, className, onClick, props }: any) => {
    ...
    return (
        <div className={cn("scale-95 active:scale-100 flex items-center py-2 px-3 space-x-2 hover:bg-slate-800 rounded-lg cursor-pointer", className)} onClick={onClick} {...props}>
            {children}
        </div>
    )
}