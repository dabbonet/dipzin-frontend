  ...
  'inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-semibold transition-all focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none ',
  {
    variants: {
      ...
      size: {
        ...
        sm: 'gap-1 rounded-xl px-3 py-2 text-sm',
        md: 'gap-2 rounded-xl px-3.5 py-2.5 text-sm',
        lg: 'gap-1.5 rounded-xl px-4 py-2.5 text-base',
        xl: 'gap-1.5 rounded-xl px-[1.375rem] py-2.5 text-base',
        '2xl': 'gap-2.5 rounded-xl px-[1.125rem] py-2.5 text-lg',
        '3xl': 'gap-2.5 rounded-xl px-[1.125rem] py-3 text-2xl',
      },
      isIconOnly: {
        true: '',
        false: '',
    ...
    compoundVariants: [
      { isIconOnly: true, size: 'default', className: 'size-10 min-w-10' },
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({
    className, variant, size, asChild = false, isIconOnly = false, ...props
  }, ref) => {
