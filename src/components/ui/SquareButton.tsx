...
const SquareButton: React.FC<ActionButtonProps> & {
  ...
  return (
    <button
      className={cn("scale-90 active:scale-100 hover:scale-95 duration-400 transition-transform flex-grow-0 flex-shrink-0 w-[5.5rem] h-[4.5rem] relative rounded-xl bg-slate-800/60 border-[2.5px] border-transparent hover:border-slate-700 cursor-pointer group focus:border-orange-500", className)}
      {...props}
    >
      {children}
    </button>
