...
const Navbar: FC = () => {
  ...
  return (
    <header className="w-full flex justify-between fixed items-start text-white pt-8 px-5 lg:px-10 z-20 top-0 bg-gradient-to-b from-slate-950/80 to-slate-950/0">
      <Link href="/ios" shallow className="text-lg lg:text-2xl flex flex-col ">
        <Image
          ...
          unoptimized
        />
        <span className=" text-[9px] bg-gradient-to-r from-aqua-400 to-aqua-400/70 w-fit mt-1.5 leading-none text-aqua-950 px-1.5 py-1 rounded-[5px]">
          BETA
        </span>
      </Link>
...
const Menu = (user) => {
  ...
  if (!isMobile && !isTablet) {
    if (user.user)
      return <SparkleButton href="/pricing">Unlock More!</SparkleButton>;
    ...
    return (
      <div className="ml-6">
        <button onClick={handleMenuClick}>
          <img src="/images/assets/menu.svg" alt="menu" title="menu" />
        </button>
      </div>
    );
  } else {
      ...
      >
        <div className="flex h-[90%] p-2 px-4 max-w-[800px] flex-col mt-10">
          <div
            ...
            className="absolute top-0 left-0 mt-8 ml-8"
          >
            <button>
          ...
          <div className=" flex gap-x-4 mb-2">
            ...
            <div className=" flex flex-col gap-3">
              ...
              >
                <img
                  src="/images/assets/video.svg"
                  className=" w-[266px] h-[148px] -ml-0"
                  alt=""
                />
              </button>
            </div>
          ...
          <div className=" flex gap-x-2 mt-3 text-xs justify-center">
            <Link
              href="/privacy"
              className=" text-slate-300  hover:text-aqua-500"
            >
              Privacy Policy
            </Link>
            <Link
  ...
  return (
    <div className="flex-shrink-0 w-[300px] bg-slate-900 hover:bg-slate-800 rounded-2xl p-4 gap-1">
      ...
      <p className="text-slate-400 text-xs">
        Work from your browser with our lightweight extension.
      </p>
      <span className=" bg-lime-100 rounded-md  px-1 text-lime-900">
        In Progress
      </span>
    </div>
  );
...
const Resources = () => {
  return (
    <div className=" flex items-start gap-x-2">
      <img
        className="mt-2 w-4 text-white"
        src="/images/assets/refresh.svg"
        alt="refresh"
        title="refresh"
      />
      <div>
        <Link href="/" className="text-base hover:text-aqua-500">
          Stream
        </Link>
        <p className=" text-slate-400 text-[10px]">
          The main gate to discover dipzin <br /> platform.
        </p>
      </div>
    </div>
  );
