...
const HomeNavigator: FC = () => {
  ...
  return path.startsWith("/search") ? (
    <div className="flex space-x-6 items-center mt-8 h-10 mx-auto max-w-[90%]">
      <span className="text-slate-900 dark:text-slate-100 text-[2rem] font-light origin-left transform transition-all duration-500 opacity-100">
        Search Results
      </span>
    </div>
  ) : (
    <div className="flex space-x-6 items-center mt-8 h-10 mx-auto  max-w-[90%]">
      <>
        <Link
          href={platforms ? `/${slug()}` : "/ios"}
          className="cursor-pointer duration-500 flex items-center"
        >
          <span
            className={cn(
              "text-slate-900 dark:text-slate-100 text-[2rem] font-normal tracking-wide origin-left transform transition-all duration-500",
              " text-[1.8rem] md:text-[2.2rem] opacity-100"
            )}
          >
            Stream
          </span>
