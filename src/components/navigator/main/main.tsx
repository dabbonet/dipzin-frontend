...
const MainNavigator = ({ type }: any) => {
  const { activeView, setActiveView, activeControls, setActiveControls } =
    useNavigator();
  const { selectedImages, setSelectedImages } = useSelcetedImages();
  const { filters, setFilters, searchKeyword, setSearchKeyword } =
    useContentDiscovery();
  ...
  useEffect(() => {
    if (
      ...
      (path.startsWith("/search") || path.startsWith("/app"))
    ) {
  ...
  return (
    <div
      ref={wrapperRef}
      className="flex w-fit items-center h-full bg-slate-900 rounded-full"
    >
              ...
              selectedImages.images.length !== 0 && (
                <div className=" flex gap-x-20 flex-wrap justify-center w-fit items-center bg-slate-900 rounded-full pl-5 py-1">
                  <div className=" flex items-center text-xs md:text-base">
                    {selectedImages.images.length} selected{" "}
                    <button
                      className=" ml-2"
                      onClick={() =>
                  ...
                  <div className="pr-3">
                    <button
                      className=" md:py-1 md:px-3 px-1 py-1 rounded-2xl bg-slate-700 text-xs md:text-base flex items-center gap-1"
                      onClick={() =>
                        ImageDownloader(
                          selectedImages.appName + " Showcase",
      ...
      <div
        className={cn(
          "relative w-fit",
          activeView == "search" ? "bg-slate-950 rounded-2xl p-3" : ""
        )}
      >
          ...
          {activeControls == "menu-search" && (
            <motion.div className={cn(" flex gap-3 items-center pl-6 w-full")}>
              ...
              <motion.input
                ...
                placeholder={
                  filters.length !== 0 ? "Search More Tags..." : "Try Search!"
                }
              ...
              />
              {activeView === "menuWithSearch" && filters.length !== 0 && (
                <button
                  onClick={removeFilter}
                  className=" text-slate-500 font-medium text-sm"
                >
                  <span className=" text-slate-50 font-bold mr-1">
                    +{filters.length}
                  </span>
                  filters
                </button>
              )}
              <p className=" text-slate-500 text-sm w-20 mr-2 text-center sm:block hidden">
                {searchKeyword.length === 0 ? "CTRL K" : "Enter"}
              </p>
            </motion.div>
          )}
          <PlatformSwitcher />
...
const TagItem = ({ title, onClick }) => {
  return (
    <li
      onClick={onClick}
      className="text-slate-400 hover:text-slate-200 cursor-pointer flex items-center space-x-1 border border-slate-500 rounded-full font-light text-sm py-[0.25rem] pl-2 pr-1.5"
    >
