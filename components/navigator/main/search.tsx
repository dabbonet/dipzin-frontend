                ...
                <>
                    <div className='w-[30%] px-2 py-2  flex-col rounded-2xl bg-slate-800 scroll-py-2 snap-y scroll-smooth overflow-y-scroll scrollbar-hide'>
                        {results.map((result: any) => (
                            <motion.div 
                                key={result.id} 
                                className={clsx(selected.id == result.id && 'bg-slate-900' ,'flex items-center p-2 hover:bg-slate-900 rounded-xl space-x-3')}
...
const PreviewCard = ({ selected }: any) => {
    ...
    useEffect(() => {
        ...
        if(selected?.platform) {
            let table = "ios_showcases"
            switch (selected.platform) {
                case 1:{
                    table = "android_showcases"
                    }
                    break;
                case 4:{
                    table = "web_showcases"
            ...
            const getShowcase = async () => {
                const {data, error} = await supabase.from(table).select('showcase, slug').eq('id', selected.id)
                if(data && data[0].showcase) {
                    setShowcase(data[0].showcase)
                }
            }
            getShowcase()
        }
          ...
          <div className='w-[70%] p-2 rounded-2xl bg-slate-800'>
              ...
              <div className='flex space-x-2 overflow-x-scroll px-2 pt-2 scrollbar-hide'>
                    {selected.platform && showcase.map((item:any, index:any) => (
                        <BlurImage className='rounded-xl w-[20%]' platform={selected.platform} src={toStorageUrl("screens/" + selected.id + '/' + item)} />
                    ))}
              </div>
          </div>
        )
